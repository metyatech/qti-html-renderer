...
- Tag-triggered npm publish workflow (`.github/workflows/publish.yml`) using npm Trusted Publisher (OIDC). It runs on `v*` tags only, verifies tag/`package.json`/`package-lock.json` version agreement, runs `npm ci` / `npm audit --audit-level=high` / `npm run verify`, asserts the packed file set, rejects an already-published version, publishes via OIDC with no npm token, and re-verifies the published artifact from the registry.
